...
var require_libsql = __commonJS((exports, module) => {
  var __dirname = "/data/apps/znote/node_modules/libsql";
...
var require_utils = __commonJS((exports, module) => {
  ...
  Utils.readBigUInt64LE = function(buffer, index3) {
    const lo = buffer.readUInt32LE(index3);
    const hi = buffer.readUInt32LE(index3 + 4);
...
var require_zipFile = __commonJS((exports, module) => {
  ...
  module.exports = function(inBuffer, options) {
    ...
    function readEntries() {
      ...
      var index3 = mainHeader.offset;
      for (var i = 0;i < entryList.length; i++) {
        var tmp = index3, entry = new ZipEntry(opts, inBuffer);
        ...
        index3 += entry.header.centralHeaderSize;
        entryList[i] = entry;
    ...
    return {
      ...
      deleteEntry: function(entryName) {
        ...
        const entry = entryTable[entryName];
        const index3 = entryList.indexOf(entry);
        if (index3 >= 0) {
          entryList.splice(index3, 1);
...
var notebooks = sqliteTable("notebooks", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var notes = sqliteTable("notes", {
  ...
  index("idx_notes_user_notebook").on(table.user_id, table.notebook_id),
  index("idx_notes_user_deleted").on(table.user_id, table.is_deleted),
  index("idx_notes_user_pinned").on(table.user_id, table.is_pinned)
]);
var noteVersions = sqliteTable("note_versions", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var files = sqliteTable("files", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  file_id: text("file_id").notNull().unique(),
  user_id: integer("user_id").notNull(),
...
var enPassword = (username, password) => {
  return pbkdf2Sync(password, username, 1e5, 32, "sha512").toString("hex");
};
var randomString = (length = 16) => {
...
// backend/api/system.ts
var checkSystemInitialized = async () => {
  const user = await db.select({ id: users.id }).from(users).limit(1).get();
  return !!user;
};
var getSystemStatus = async (c) => {
  return c.json({
    code: 200,
    msg: "success",
    data: {
...
// backend/api/setting.ts
import { eq } from "drizzle-orm";
var DEFAULT_USER_SETTINGS = {
  dashboard_home: "overview",
  dashboard_layout: "default"
};
var getUserSettingValue = async (uid) => {
  const row = await db.select({ value: userSettings.value }).from(userSettings).where(eq(userSettings.uid, uid)).get();
  if (!row?.value) {
...
var setUserSetting = async (c) => {
  ...
  const existing = await db.select({ id: userSettings.id }).from(userSettings).where(eq(userSettings.uid, uid)).get();
  if (existing) {
    await db.update(userSettings).set({ value, updated_at: new Date }).where(eq(userSettings.uid, uid)).run();
  } else {
    await db.insert(userSettings).values({ uid, value }).run();
  }
  return c.json({
    code: 200,
    msg: "success",
...
var register = async (c) => {
  ...
  const userCount = await db.select({ count: sql`count(*)` }).from(users).get();
  ...
  const existingUsername = await db.select({ id: users.id }).from(users).where(eq2(users.username, username)).get();
  if (existingUsername) {
    return c.json({ code: -1000, msg: "username.already.exists", data: null });
  }
  const existingEmail = await db.select({ id: users.id }).from(users).where(eq2(users.email, email)).get();
  if (existingEmail) {
...
var login = async (c) => {
  ...
  if (username.includes("@")) {
    ...
    const userByEmail = await db.select({ username: users.username }).from(users).where(eq2(users.email, username)).get();
    if (!userByEmail) {
  ...
  const user = await db.select({
    ...
    email: users.email,
    role: users.role
  }).from(users).where(and(eq2(users.username, actualUsername), eq2(users.password, encryptedPassword), eq2(users.status, "active"))).get();
  if (!user) {
    return c.json({ code: -1000, msg: "invalid.username.or.password", data: null });
  }
  const token = await createSession(user.id, user.role, c);
...
var logout = async (c) => {
  ...
  if (token) {
    await db.update(sessions).set({ status: "revoked" }).where(and(eq2(sessions.token, token), eq2(sessions.status, "active"))).run();
  }
  return c.json({ code: 200, msg: "success", data: null });
};
var changePassword = async (c) => {
  const uid = Number(c.get("uid"));
  ...
  const user = await db.select({ id: users.id }).from(users).where(and(eq2(users.id, uid), eq2(users.password, enPassword(username, old_password)), eq2(users.status, "active"))).get();
  if (!user) {
...
var listUsers = async (c) => {
  ...
  return c.json({ code: 200, msg: "success", data: users2 });
};
var resetUserPassword = async (c) => {
  const { id } = await c.req.json();
  if (!Number.isInteger(id) || id <= 0) {
    return c.json({ code: -1000, msg: "user.id.invalid", data: null });
  }
  const user = await db.select({ id: users.id, username: users.username, role: users.role }).from(users).where(eq2(users.id, id)).get();
  if (!user) {
    return c.json({ code: -1000, msg: "user.not.found", data: null });
  }
  if (user.role !== "user") {
  ...
  await db.update(users).set({ password: enPassword(user.username, newPassword), updated_at: new Date }).where(eq2(users.id, user.id)).run();
  return c.json({
    code: 200,
...
var resetAdminPassword = async (c) => {
  const admin = await db.select({ id: users.id, username: users.username }).from(users).where(eq2(users.role, "admin")).orderBy(users.id).limit(1).get();
  if (!admin) {
    return c.text("\u7BA1\u7406\u5458\u8D26\u53F7\u4E0D\u5B58\u5728", 404);
  }
  const resetFilePath = join2(process.cwd(), "data", "reset.txt");
  const resetFileExists = await Bun.file(resetFilePath).exists();
  if (!resetFileExists) {
    return c.text("Forbidden", 403);
  }
  const password = randomString(12);
  await db.update(users).set({ password: enPassword(admin.username, password), updated_at: new Date }).where(eq2(users.id, admin.id)).run();
  return c.text(`\u7BA1\u7406\u5458\u65B0\u5BC6\u7801: ${password}`);
};

// backend/api/notebook.ts
import { and as and2, eq as eq3, inArray, isNull } from "drizzle-orm";
var buildNotebookTree = (flatList) => {
...
var checkNodeOwnership = async (nodeId, userId) => {
  const node = await db.select({ id: notebooks.id }).from(notebooks).where(and2(eq3(notebooks.id, nodeId), eq3(notebooks.user_id, userId))).get();
  return !!node;
};
var wouldCreateCycle = async (nodeId, newParentId) => {
  let currentId = newParentId;
  while (currentId !== null) {
    if (currentId === nodeId)
      return true;
    const node = await db.select({ parent_id: notebooks.parent_id }).from(notebooks).where(eq3(notebooks.id, currentId)).get();
    if (!node)
      return false;
...
var listNotebooks = async (c) => {
  const uid = Number(c.get("uid"));
  const notebooks2 = await db.select().from(notebooks).where(eq3(notebooks.user_id, uid)).orderBy(notebooks.sort_order).all();
  const tree = buildNotebookTree(notebooks2);
  return c.json({
    code: 200,
...
var getTopLevelNotebooks = async (c) => {
  const uid = Number(c.get("uid"));
  const notebooks2 = await db.select().from(notebooks).where(and2(eq3(notebooks.user_id, uid), isNull(notebooks.parent_id))).orderBy(notebooks.sort_order).all();
  return c.json({
    code: 200,
...
var updateNotebook = async (c) => {
  ...
  const node = await db.select().from(notebooks).where(and2(eq3(notebooks.id, id), eq3(notebooks.user_id, uid))).get();
  if (!node) {
    return c.json({
      code: -1000,
  ...
  const result = await db.update(notebooks).set(updates).where(and2(eq3(notebooks.id, id), eq3(notebooks.user_id, uid))).returning().get();
  return c.json({
    code: 200,
...
var sortNotebooks = async (c) => {
  ...
  for (const item of items) {
    if (!item || typeof item.id !== "number" || typeof item.sort_order !== "number" || !Number.isFinite(item.sort_order)) {
      return c.json({
        code: -1000,
  ...
  const firstNode = await db.select({ parent_id: notebooks.parent_id }).from(notebooks).where(and2(eq3(notebooks.id, items[0].id), eq3(notebooks.user_id, uid))).get();
  if (!firstNode) {
    return c.json({
      code: -1000,
  ...
  await db.transaction(async (tx) => {
    for (const item of items) {
      await tx.update(notebooks).set({
        sort_order: item.sort_order,
        updated_at: now
      }).where(and2(eq3(notebooks.id, item.id), eq3(notebooks.user_id, uid)));
    }
  });
  const notebooks2 = await db.select().from(notebooks).where(and2(eq3(notebooks.user_id, uid), parentId === null ? isNull(notebooks.parent_id) : eq3(notebooks.parent_id, parentId))).orderBy(notebooks.sort_order).all();
  return c.json({
    code: 200,
...
var collectDescendantIds = async (rootIds) => {
  const allIds = new Set(rootIds);
  let currentIds = [...rootIds];
  while (currentIds.length > 0) {
    const children = await db.select({ id: notebooks.id }).from(notebooks).where(inArray(notebooks.parent_id, currentIds)).all();
    currentIds = [];
...
var deleteNotebooks = async (c) => {
  ...
  const ownedRows = await db.select({ id: notebooks.id }).from(notebooks).where(and2(inArray(notebooks.id, ids), eq3(notebooks.user_id, uid))).all();
  ...
  await db.transaction(async (tx) => {
    const now = new Date;
    const notesResult = await tx.update(notes).set({
      is_deleted: 1,
      deleted_at: now,
      updated_at: now
    }).where(and2(inArray(notes.notebook_id, idList), eq3(notes.user_id, uid))).returning({ id: notes.id }).all();
    deletedNotes = notesResult.length;
    const notebookResult = await tx.delete(notebooks).where(and2(inArray(notebooks.id, idList), eq3(notebooks.user_id, uid))).returning({ id: notebooks.id }).all();
...
// backend/api/note.ts
import { and as and4, asc, desc as desc2, eq as eq5, inArray as inArray2, sql as sql2 } from "drizzle-orm";
...
var checkNotebookOwnership = async (notebookId, userId) => {
  const node = await db.select({ id: notebooks.id }).from(notebooks).where(and3(eq4(notebooks.id, notebookId), eq4(notebooks.user_id, userId))).get();
  return !!node;
};
var checkNoteOwnership = async (noteId, userId) => {
  const note = await db.select({ id: notes.id }).from(notes).where(and3(eq4(notes.id, noteId), eq4(notes.user_id, userId))).get();
  return !!note;
};

// backend/utils/file.ts
...
var FILES_BASE_PATH = "./data/files";

// backend/api/note.ts
var listNotes = async (c) => {
  const uid = Number(c.get("uid"));
  ...
  const notes2 = await db.select().from(notes).where(and4(eq5(notes.user_id, uid), eq5(notes.notebook_id, notebookId), eq5(notes.is_deleted, 0))).orderBy(desc2(notes.is_pinned), asc(notes.sort_order), desc2(notes.created_at)).all();
  return c.json({
    code: 200,
...
var updateNote = async (c) => {
  ...
  const oldNote = await db.select({ title: notes.title, content: notes.content }).from(notes).where(and4(eq5(notes.id, id), eq5(notes.user_id, uid))).get();
  if (!oldNote) {
    return c.json({
      code: -1000,
  ...
  const result = await db.transaction(async (tx) => {
    ...
    return await tx.update(notes).set(updates).where(and4(eq5(notes.id, id), eq5(notes.user_id, uid))).returning().get();
  });
  return c.json({
    code: 200,
...
var deleteNote = async (c) => {
  ...
  const result = await db.update(notes).set({
    ...
    deleted_at: now,
    updated_at: now
  }).where(and4(eq5(notes.id, id), eq5(notes.user_id, uid))).returning().get();
  return c.json({
    code: 200,
...
var sortNotes = async (c) => {
  ...
  const firstNote = await db.select({ notebook_id: notes.notebook_id }).from(notes).where(and4(eq5(notes.id, items[0].id), eq5(notes.user_id, uid))).get();
  if (!firstNote) {
    return c.json({
      code: -1000,
  ...
  await db.transaction(async (tx) => {
    for (const item of items) {
      await tx.update(notes).set({
        sort_order: item.sort_order,
        updated_at: now
      }).where(and4(eq5(notes.id, item.id), eq5(notes.user_id, uid)));
    }
  });
  const notes2 = await db.select().from(notes).where(and4(eq5(notes.user_id, uid), eq5(notes.notebook_id, notebookId), eq5(notes.is_deleted, 0))).orderBy(desc2(notes.is_pinned), asc(notes.sort_order), desc2(notes.created_at)).all();
  return c.json({
...
var getNoteById = async (c) => {
  ...
  const note = await db.select().from(notes).where(and4(eq5(notes.id, noteId), eq5(notes.user_id, uid))).get();
  if (!note) {
    return c.json({
      code: -1000,
...
var permanentDeleteNote = async (c) => {
  const uid = Number(c.get("uid"));
  const payload = await c.req.json();
  const { id } = payload || {};
  if (!id || typeof id !== "number") {
  ...
  const note = await db.select({ content: notes.content }).from(notes).where(and4(eq5(notes.id, id), eq5(notes.user_id, uid))).get();
  if (!note) {
    return c.json({
      code: -1000,
  ...
  const versions = await db.select({ content: noteVersions.content }).from(noteVersions).where(eq5(noteVersions.note_id, id)).all();
  const contents = [note.content, ...versions.map((v) => v.content)].filter(Boolean);
  const fileIdSet = new Set;
  const reg = /zn-[A-Za-z0-9]{8}/g;
  for (const text2 of contents) {
  ...
  await db.transaction(async (tx) => {
    await tx.delete(noteVersions).where(and4(eq5(noteVersions.note_id, id), eq5(noteVersions.user_id, uid)));
    if (fileIdSet.size > 0) {
      await tx.delete(files).where(and4(inArray2(files.file_id, [...fileIdSet]), eq5(files.user_id, uid)));
    }
    await tx.delete(notes).where(and4(eq5(notes.id, id), eq5(notes.user_id, uid)));
  });
...
var importZip = async (c) => {
  ...
  const tmpDir = join4("/tmp", `zest_import_${uid}_${Date.now()}`);
  ...
  try {
    ...
    const topDirs = entries.filter((e) => {
      try {
        return statSync(join4(extractDir, e)).isDirectory();
      } catch {
        return false;
      }
    });
    const topMds = entries.filter((e) => {
      try {
        return e.endsWith(".md") && statSync(join4(extractDir, e)).isFile();
      } catch {
        return false;
      }
    });
...
// backend/routers.ts
var allowedOrigin = Bun.env.ZNOTE_CORS_ORIGIN?.trim() || "*";
var corsOptions = {
